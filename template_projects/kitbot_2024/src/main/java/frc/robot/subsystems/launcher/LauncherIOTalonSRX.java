...
public class LauncherIOTalonSRX implements LauncherIO {
  ...
  public LauncherIOTalonSRX() {
    ...
    tryUntilOkV5(5, () -> launchMotor.configAllSettings(config));
    tryUntilOkV5(5, () -> feedMotor.configAllSettings(config));
  }

  @Override
  public void updateInputs(LauncherIOInputs inputs) {
    inputs.launchAppliedVolts = launchMotor.getMotorOutputVoltage();
    inputs.launchCurrentAmps = launchMotor.getSupplyCurrent();

    inputs.feedAppliedVolts = feedMotor.getMotorOutputVoltage();
    inputs.feedCurrentAmps = feedMotor.getSupplyCurrent();
  }
