...
public class LauncherIOSim implements LauncherIO {
  private DCMotorSim launchSim =
      new DCMotorSim(
          LinearSystemId.createDCMotorSystem(DCMotor.getCIM(1), 0.025, 4.0),
          DCMotor.getCIM(1),
          0.0001);
  private DCMotorSim feedSim =
      new DCMotorSim(
          LinearSystemId.createDCMotorSystem(DCMotor.getCIM(1), 0.025, 4.0),
          DCMotor.getCIM(1),
          0.0001);
  ...
  public void updateInputs(LauncherIOInputs inputs) {
    ...
    inputs.launchCurrentAmps = launchSim.getCurrentDrawAmps();
