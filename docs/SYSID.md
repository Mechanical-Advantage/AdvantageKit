...
> Note: Device logging systems like AdvantageScope's [URCL](https://github.com/Mechanical-Advantage/AdvantageScope/blob/main/docs/REV-LOGGING.md) (Unofficial REV-Compatible Logger) and CTRE's [signal logger](https://pro.docs.ctr-electronics.com/en/latest/docs/api-reference/api-usage/signal-logging.html) can be used to collect data instead of AdvantageKit. In this case, please following the instructions in the corresponding documentation.
...
> Note: AdvantageKit log files **should NOT be used directly with SysId**. Follow the instructions below to convert it to the correct format.
...
2. Set the format to "WPILOG" and the timestamps to "AdvantageKit Cycles". For large log files, enter the prefixes for only the fields and tables necessary for SysId analysis (see the [export options](https://github.com/Mechanical-Advantage/AdvantageScope/blob/main/docs/EXPORT.md#options) documentation for details).
