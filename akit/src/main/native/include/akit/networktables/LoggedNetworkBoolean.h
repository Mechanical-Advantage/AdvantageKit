...
class LoggedNetworkBoolean : public LoggedNetworkInput, public inputs::LoggableInputs {
    public:
    LoggedNetworkBoolean(std::string key) : key{key}, entry{::nt::NetworkTableInstance::GetDefault().GetBooleanTopic(key).GetEntry(false)}, value{defaultValue} {
        // Logger.registerDashboardInput(this)
    }

    LoggedNetworkBoolean(std::string key, bool defaultValue) : LoggedNetworkBoolean{key} {
