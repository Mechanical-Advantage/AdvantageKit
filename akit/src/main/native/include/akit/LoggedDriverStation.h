...
public:
	void saveToLog(LogTable table) {
		...
		for (int i = 0; i < frc::DriverStation::kJoystickPorts; i++) {
			...
			std::vector < int16_t
					> povValues { rawPovValues.begin(), rawPovValues.begin()
							+ povCount };
			...
			std::vector < uint8_t
					> axisTypes { rawAxisTypes.begin(), rawAxisTypes.begin()
							+ axisCount };
	...
	void replayFromLog(LogTable table) {
		frc::sim::DriverStationSim::SetAllianceStationId(
				table.get("AllianceStation", 0));
		frc::sim::DriverStationSim::SetEventName(table.get("EventName", ""));
		frc::sim::DriverStationSim::SetGameSpecificMessage(
				table.get("GameSpecificMessage", ""));
		frc::sim::DriverStationSim::SetMatchNumber(table.get("MatchNumber", 0));
		frc::sim::DriverStationSim::SetReplayNumber(
				table.get("ReplayNumber", 0));
		...
		frc::sim::DriverStationSim::SetAutonomous(
				table.get("Autonomous", false));
		frc::sim::DriverStationSim::SetTest(table.get("Test", false));
		frc::sim::DriverStationSim::SetEStop(table.get("EmergencyStop", false));
		frc::sim::DriverStationSim::SetFmsAttached(
				table.get("FMSAttached", false));
		...
		for (int id = 0; id < frc::DriverStation::kJoystickPorts; id++) {
			...
			std::vector<int> povValues = joystickTable.get("POVs",
					std::vector<int> { });
			frc::sim::DriverStationSim::SetJoystickPOVCount(id,
					povValues.size());
			...
			std::vector<float> axisValues = joystickTable.get("AxisValues",
					std::vector<float> { });
			std::vector<int> axisTypes = joystickTable.get("AxisTypes",
					std::vector<int> { });
			frc::sim::DriverStationSim::SetJoystickAxisCount(id,
					axisValues.size());
			for (int i = 0; i < axisValues.size(); i++) {
				frc::sim::DriverStationSim::SetJoystickAxis(id, i,
						axisValues[i]);
