...
public:
	void saveToLog(LogTable table) {
        ...
        std::vector<nt::ConnectionInfo> ntConnections = nt::NetworkTableInstance::GetDefault().GetConnections();
        ...
        for (int i = 0; i < ntConnections.size(); i++) {
            ...
            LogTable ntClientTable = ntClientsTable.getSubtable(ntConnections[i].remote_id);
            ...
            ntIntBuffer = std::bit_cast<std::array<std::byte, 4>>(ntConnections[i].protocol_version);
