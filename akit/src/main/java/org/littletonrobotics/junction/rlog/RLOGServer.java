      ...
      while (true) {
        try {
          ...
          synchronized (socketsLock) {
            sockets.add(socket);
            lastHeartbeats.add(RobotController.getMonotonicTime() / 1_000_000_000.0);
          }
          System.out.println(
          ...
          for (int i = 0; i < sockets.size(); i++) {
            ...
            try {
              ...
              if (inputStream.available() > 0) {
                ...
                lastHeartbeats.set(i, RobotController.getMonotonicTime() / 1_000_000_000.0);
              }

              // Close connection if socket timed out
              if (RobotController.getMonotonicTime() / 1_000_000_000.0 - lastHeartbeats.get(i)
